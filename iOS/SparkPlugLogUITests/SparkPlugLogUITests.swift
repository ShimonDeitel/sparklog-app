...
final class SparkPlugLogUITests: XCTestCase {
    ...
    func testFreeLimitTriggersPaywall() {
        ...
        for _ in 0..<9 {
            app.buttons["addButton"].tap()
            if app.staticTexts["Spark Plug Log Pro"].waitForExistence(timeout: 1) {
                break
            }
            let saveButton = app.buttons["saveButton"]
        ...
        XCTAssertTrue(app.staticTexts["Spark Plug Log Pro"].waitForExistence(timeout: 3))
    }
